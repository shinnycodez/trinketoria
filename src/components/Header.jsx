...
const Header = () => {
  ...
  const adminEmails = ["mueezimran1@gmail.com", "buttmaham771@gmail.com"];

  return (
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#fdcddb] bg-[#fdcddb] px-4 md:px-10 py-3 relative">
        <div className="flex items-center gap-4 md:gap-8">
          <Link to="/" className="flex items-center gap-2 md:gap-4 text-[#141414]">
            <h2 className="text-[#141414] text-lg font-bold leading-tight tracking-[-0.015em]">TRINKETORIA</h2>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-9">
            ...
            <Link to="/products?category=Floral bracelets" className="text-[#141414] text-sm font-medium transition">Floral Bracelets ˚⊹♡</Link>
            <Link to="/products?category=rings" className="text-[#141414] text-sm font-medium  transition">
Rings ⋆｡𖦹°</Link>
            <Link to="/products?category=keychains" className="text-[#141414] text-sm font-medium  transition">Earings ˚⋆𐙚｡</Link>
        ...
        <div className="flex flex-1 justify-end gap-4 md:gap-8">
          <label className="hidden md:flex flex-col min-w-40 !h-10 max-w-64">
            <div className="flex w-full flex-1 items-stretch rounded-lg h-full">
              <div className="text-neutral-500 flex border-none bg-[#fdcddb] items-center justify-center pl-4 rounded-l-lg border-r-0">
...
#fdcddb] focus:border-none h-full placeholder:text-neutral-500 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
                value={searchQuery}
...
#fdcddb] text-[#141414] hover:bg-[#fdcddb] transition">🛒 Cart</button>
            {user ? (
              <>
                <button onClick={handleLogout} className="px-4 h-10 rounded-lg bg-[#
#fdcddb] text-[#141414] text-sm font-semibold hover:bg-[#
#fdcddb] transition">Logout</button>
                <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10 border-2 border-[#fdcddb]"
                  style={{
...
#fdcddb] text-white text-sm font-semibold hover:opacity-90 transition"
              >
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-50 border-t border-[#ededed]">
            <div className="px-4 py-2 space-y-2">
              <div className="flex items-center gap-2 rounded-lg bg-[#fdcddb] px-3 py-2">
              ...
              <Link to="/products?category=Floral bracelets" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Floral Bracelets ˚⊹♡</Link>
              <Link to="/products?category=rings" onClick={() => setIsMenuOpen(false)} className="blocsk px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Rings ⋆｡𖦹°
</Link>
              <Link to="/products?category=Earings" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">
earings ˚⋆𐙚｡
</Link>
              ...
              <Link to="/products?category=Stretchable bracelets" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Stretchable bracelets ⋆⭒˚.⋆
</Link>
